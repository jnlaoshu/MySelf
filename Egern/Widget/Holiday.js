...
export default async function(ctx) {
  ...
  const getFests = (y) => {
    ...
    return {
      legal: [
        ["元旦",YMD(y,1,1),1], ["春节",l2s(1,1),3], 
        ["成都春假",YMD(y, 4, Lunar.term(y, 7).getUTCDate() - 3),3], // 成都市中小学春假：清明节前3天
        ["清明节",term(7),1], 
        ["劳动节",YMD(y,5,1),1], ["端午节",l2s(5,5),1], ["高考",YMD(y,6,7),2], 
        ["中秋节",l2s(8,15),1], ["国庆节",YMD(y,10,1),3], 
        ["成都秋假",wDay(11,2,3),3] // 成都市中小学秋假：11月第2个周三开始
      ],
      folk: [
  ...
  const result = { legal: [], folk: [], intl: [] };

  ["legal", "folk", "intl"].forEach(cat => {
    const list = f1[cat].concat(f2[cat]);
    list.forEach(([name, dateStr, duration]) => {
      if (!dateStr) return;
  ...
  const format = (cat) => result[cat].sort((a,b)=>a.diff-b.diff).slice(0,3)
    .map(i => i.diff === 0 ? `🎉${i.name}` : `${i.name} ${i.diff}天`).join(" , ");
  ...
  return {
    ...
    children: [
      { type: 'spacer', length: 10 },
      {
        ...
        children: [
          { type: 'image', src: 'sf-symbol:timer', color: '#FFFFFF', width: 18, height: 18 },
          { type: 'spacer', length: 4 },
          { type: 'text', text: '时光倒数', font: { size: 17, weight: 'bold' }, textColor: '#FFFFFF' },
          { type: 'text', text: titleAddon, font: { size: 15, weight: 'bold' }, textColor: '#FFD426' }
        ]
      },
      { type: 'spacer', length: 14 },
      {
        ...
        children: [
          { i: "building.columns.fill", col: "#FFD426", n: "法定", t: format("legal") },
          { i: "moon.stars.fill", col: "#FF9570", n: "民俗", t: format("folk") },
          { i: "globe.americas.fill", col: "#59B2FF", n: "国际", t: format("intl") }
          ...
          children: [
            ...
            { type: 'text', text: `${cat.n}：${cat.t}`, font: { size: 13, weight: 'medium' }, textColor: '#F5F5F7', lineSpacing: 4 }
          ]
        }))
      },
      { type: 'spacer' }
