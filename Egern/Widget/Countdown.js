...
export default async function (ctx) {
  ...
  if (isSmall) {
    let allItems = [];
    Object.keys(result).forEach(cat => allItems.push(...result[cat]));
    pinnedMap.forEach((diff, name) => allItems.push({ name, diff }));
    allItems.sort((a, b) => a.diff - b.diff);

    const smallText = allItems.slice(0, 11).map(i => formatItemStr(i.name, i.diff)).join("，");
  ...
  rightHeaderElements.push(mkText(refreshTimeStr, layoutConfig.topFz, "medium", C.muted));
