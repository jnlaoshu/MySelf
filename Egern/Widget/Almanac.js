...
export default async function(ctx) {
  ...
  const Lunar = {
    ...
    getTerm(y, n) {
      // 修复核心：移除原代码结尾错误的 +8 小时偏移。Date.UTC 已经将 1900 锚点基准正确对齐。
      const t = new Date(
  ...
  for (let i = 0; i < allTerms.length; i++) {
    ...
    // 如果今天是节气 (diffDays === 0)，倒计时系统自动跳过今天，显示下一个节气
    if (diffDays >= 0) {
  ...
  // ── 渲染核心排版 (已恢复 maxLines: 1 防止错位，精准适配中号宽度) ──
  ...
  const createRowFactory = (config) => (raw, icon, color, label, contentColor = C.sub) => {
    ...
    return lines.map((lineStr, idx) => ({
      ...
      children: [
        ...
        // 关键修复：必须加回 maxLines: 1，强制依赖上方切词逻辑切行，禁止底层乱缠
  ...
  const layoutConfig = {
    ...
    icz: isLg ? 15 : 13,
    lw: isLg ? 60 : 52,
    // 关键修复：将中号 maxW 调整为 24（约10个中文字符），触发极度优美的多行悬挂缩进
    maxW: isLg ? 38 : 24,
    headerFz: isLg ? 17 : 15,
