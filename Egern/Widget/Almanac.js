...
- ==========================================
- 📌 代码名称: 📅 岁时黄历 (节气流转全览版)
- ✨ 主要功能: 实时推算公农历、干支生肖、时辰；调用 API 获取宜忌、冲煞与运势；
- ```
          内置历法计算节气倒数；采用弹性布局，适配深浅色模式。
  ```
- ✨ 专属定制: 右上角支持”星座”与”周次”双模式动态切换显示，
- ```
          支持通过模块环境变量 ASTRO_OR_WEEK 控制。
  ```
- ⏱️ 更新时间: 2026.03.25 13:45
- ==========================================
  */
...
- 避免一次性生成三年 72 个节气对象。
  */
  ...
  const results  = [];
  let   year     = Y;
  let   termIdx  = (M - 1) * 2 + 1;          // 从当月第一个节气开始扫
