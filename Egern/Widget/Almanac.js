...
export default async function(ctx) {
  ...
  const THEME_ACCENT_GOLD = { light: '#FF9500', dark: '#FFD426' };
  const THEME_ACCENT_GREEN = { light: '#34C759', dark: '#30D158' };
  ...
  return {
    ...
    children: [
      ...
      { type: 'stack', direction: 'row', alignItems: 'center', gap: 8, children: [
          ...
          { type: 'text', text: `${Y}年${M}月${D}日 星期${WEEK[now.getDay()]} ${obj.astro}`, font: { size: 'headline', weight: 'bold' }, textColor: TEXT_MAIN, maxLines: 1, minScale: 0.8 }
      ]},
      { type: 'spacer', length: 6 }, 
      { type: 'text', text: `${obj.gz}(${obj.ani})年 ${obj.cn}${obj.term ? ` ✨今日${obj.term}` : ` · 当前${currentTerm}`}`, font: { size: 14, weight: 'medium' }, textColor: THEME_ACCENT_GOLD, maxLines: 1, minScale: 0.8 },
      { type: 'spacer', length: 8 }, 
