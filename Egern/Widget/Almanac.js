 ...
 * ==========================================
 * 📌 代码名称: 📅 岁时黄历 (节气流转全览版)
 * ✨ 主要功能: 实时公农历、干支、宜忌；splitText 多节点分行彻底解决 iOS 长文截断；左侧标签 width:52 全局对齐。
 * 🔗 引用链接: https://raw.githubusercontent.com/jnlaoshu/MySelf/master/Egern/Widget/Almanac.js
 * ⏱️ 更新时间: 2026.03.26 11:45
 * ==========================================
 */
...
export default async function(ctx) {
  ...
  const getVal     = (...keys) => { for (const k of keys) if (apiData[k]) return apiData[k]; return ""; };
  ...
  // ==================== 文字分行（多节点，规避 iOS \n 不折行） ====================
  // maxW: 宜忌第一行用 52 尽量铺满，续行用 56 可撑满整行（无标签占位）
  ...
  // ==================== 行渲染（width:52 对齐，alignItems:start 支持多行） ====================
  ...
  // 宜忌：第一行 maxW=52 尽量铺满，余下内容合并为第二行
  const buildYiJiRows = (raw, icon, color, label) => {
    if (!raw) return [];
    const [l0, ...rest] = splitText(raw, 52);
    const rows = [];
