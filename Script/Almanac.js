/*
 * 今日黄历&节假日倒数（完美修复版）
 * 修复：解决年初（农历年尾）不显示小年、除夕等上一农历年节日的问题
 * 更新：2026.02.10
 */
(async () => {
  // 1. 基础环境 (UTC+8)
  ...
  const merge = (list, limit = 3) => {
    ...
    for (const [name, dateStr] of list) {
        ...
        // diff < 0 表示已过期，这里会过滤掉去年的旧节日，但会保留今年初属于去年农历的节日
        if (diff < 0) continue;
        let sortKey = diff;
  ...
  try {
    ...
    // 【修复】获取 Y-1, Y, Y+1 三年的数据，防止年初时漏掉农历旧年的尾巴（如小年、除夕）
    const f0 = getFests(Y - 1); // 去年 (覆盖今年初的农历尾)
    const f1 = getFests(Y);     // 今年
    const f2 = getFests(Y + 1); // 明年
