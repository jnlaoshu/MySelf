 ...
 * 今日黄历&节假日倒数（终极修复版）
 * 修复：
 * 1. 严格还原 Lunar.info 数据，确保农历年份、月份、日期显示绝对正确。
 * 2. 引入 Y-1 年份扫描，完美解决“小年”、“除夕”等处于公历年初但属于农历上一年的节日显示问题。
 * 3. 增加当日节日置顶显示功能。
 * 更新：2026.02.10
 */
(async () => {
  ...
  try {
    ...
    // 【核心修复】同时获取去年、今年、明年的节日
    // 小年和除夕在公历年初（1月/2月），但属于农历上一年的尾巴。
    // 比如2026年2月10日是农历2025年腊月二十三。必须调用 getFests(2025) 才能计算出这个日期。
