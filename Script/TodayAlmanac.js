/*
 * 今日黄历&节假日倒数 (V32.3 强力兼容版)
 * ✅ 修复：解决修改后无法获取数据的问题 (增加斜杠/和数字格式支持)
 * ✅ 安全：杜绝获取到错误日期 (如1月17日获取到2月16日)
 */
(async () => {
  // 1. 基础环境 (UTC+8)
  ...
  const MATCH_LIST = [
    `${Y}-${P(M)}-${P(D)}`, // 2026-01-17 (标准横杠)
    `${Y}-${M}-${D}`,       // 2026-1-17  (去零横杠)
    `${Y}/${P(M)}/${P(D)}`, // 2026/01/17 (标准斜杠)
    `${Y}/${M}/${D}`,       // 2026/1/17  (去零斜杠)
    `${Y}${P(M)}${P(D)}`,   // 20260117   (字符串或数字)
    `date":"${Y}-${P(M)}-${P(D)}` // 针对某些特殊JSON字符串
  ];
  const WEEK = "日一二三四五六";
    ...
    return new Promise(r => {
      ...
      const scan = n => {
        ...
        if (n.yi || n.suit || n.gregorian) {
            ...
            // 核心修复：检查 dStr 是否包含 MATCH_LIST 中的任意一种格式
            // 这样既支持了斜杠日期，又因为包含了年份和月份，绝对不会匹配到 2026-02-16
            const isMatch = MATCH_LIST.some(fmt => dStr.includes(fmt));
            
            // 兜底：如果 dStr 仅仅是数字 "17" 或 "17日"，且我们确定在 202601.json 文件中
            // 这种匹配比较危险，但如果数据源格式极其简陋，这是最后的希望
            // 我们通过排除 "-" 和 "/" 来确保它不是其他月份的完整日期
            const isSimpleDay = !dStr.includes("-") && !dStr.includes("/") && parseInt(dStr) === D;

            if (isMatch || isSimpleDay) {
                res = n;
            }
        }
