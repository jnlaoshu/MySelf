/*
 * 今日黄历&节假日倒数 (V31.0 紧急修复版)
 * ✅ 修复：补全缺失的 weekCn 变量，解决脚本报错
 * ✅ 修复：修正变量引用 (now -> NOW)，防止 undefined 错误
 * ✅ 整合：采用 V26 的生肖算法 + V29 的倒计时算法，确保全对
 */
(async () => {
  // ========== 1. 基础环境 (UTC+8) ==========
  const getNow = () => {
  ...
  const weekCn = "日一二三四五六"; // 🟢 补回变量
  
  // 匹配指纹
  const MATCH = {
    std: `${cY}-${pad(cM)}-${pad(cD)}`,
    short: `${cY}-${cM}-${cD}`,
    day: cD
  };
    ...
    return new Promise(resolve => {
      ...
      return list.find(i => {
        if (i.date) {
          ...
          if (ds === MATCH.std || ds === MATCH.short || ds.includes(MATCH.std)) return true;
        }
        if (i.day !== undefined && parseInt(i.day, 10) === MATCH.day) return true;
        return false;
      });
    });
  };
  ...
  const Lunar = {
    ...
    convert(y, m, d) {
      ...
      return { 
        gzYear: this.gan[(lYear-4)%10]+this.zhi[(lYear-4)%12], 
        animal: this.ani[(lYear-4)%12],
        monthCn: `${isLeap?"闰":""}${this.monStr[i-1]}月`,
        dayCn: lDay==10?"初十":lDay==20?"二十":lDay==30?"三十":["初","十","廿","卅"][Math.floor(lDay/10)]+this.nStr[lDay%10],
        astro: "摩羯水瓶双鱼白羊金牛双子巨蟹狮子处女天秤天蝎射手摩羯".substr(m*2 - (d < [20,19,21,21,21,22,23,23,23,23,22,22][m-1]?2:0), 2) + "座",
        term: term 
      };
    },
    
    // 农历计算：使用 V29 逻辑 (倒计时准确)
    l2s(y,m,d) { try {
  ...
  try {
    ...
    const getV = (...k) => { for(let i of k) if(dayData && dayData[i]) return dayData[i]; return ""; };
    ...
    $done({
      title: `${cY}年${pad(cM)}月${pad(cD)}日 星期${weekCn[NOW.day]} ${lObj.astro}`, // 🟢 引用 NOW.day
