...
(async () => {
  ...
  const pickTitle = (nextName, daysToNext) => {
    try {
      ...
      // 准备新占位符的数据
      const curDate = todayStr; // e.g. 2023-10-27
      const curTime = `${pad2(tnow.getHours())}:${pad2(tnow.getMinutes())}`; // e.g. 09:30

      return raw
        .replaceAll("{lunar}", titleLunar)
        .replaceAll("{solar}", titleSolar)
        .replaceAll("{next}", nextName || "")
        .replaceAll("{date}", curDate) // 替换公历日期
        .replaceAll("{time}", curTime); // 替换当前时刻
    } catch (e) {
