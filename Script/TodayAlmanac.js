...
(async () => {
  ...
  const getLunarDesc = async (lunarData) => {
    ...
    let almanacList = [];
    if (data?.data?.almanac) {
       almanacList = data.data.almanac;
    } else if (Array.isArray(data?.data) && data.data[0]?.almanac) {
       almanacList = data.data[0].almanac;
    }

    // Fix: Match by full date string (yyyy-mm-dd) instead of simple day number
    const matchDateStr = formatYmd(curYear, curMonth, curDate);
    const almanacItem = almanacList.find(i => i.date === matchDateStr);
