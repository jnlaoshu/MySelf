...
(async () => {
    ...
    const generateFestData = year => {
        ...
        return {
            ...
            term: Array.from({length:24},(_,i)=>{const m=Math.floor(i/2)+1;return [LunarCal.terms[i],formatYmd(year,m,LunarCal.getTerm(year,i+1))];})
        };
    };

    // ===================== 【核心重点：宜忌请求 局部try/catch 只影响宜忌 不影响全局】 =====================
    let almanacTxt = "";
    if (hasHttpClient) {
        try {
            const httpGet = async (url) => {
                return new Promise(resolve => {
                    $httpClient.get({url, timeout: 10000, headers: {"User-Agent": "Mozilla/5.0"}}, (err, resp, data) => resolve((!err && resp?.status ===200) ? data : null));
                });
            };
            // 绝对正确的接口路径
            const giteeApi = `https://gitee.com/zqzess/openApiData/raw/main/calendar_new/${curYear}/${curYM}.json`;
            let data = await httpGet(giteeApi);
            if(!data) data = await httpGet(`https://raw.githubusercontent.com/zqzess/openApiData/main/calendar_new/${curYear}/${curYM}.json`);
            if(data) {
                const lunarArray = JSON.parse(data);
                const todayLunar = lunarArray.find(item => item?.solar?.month === curMonth && item?.solar?.day === curDate);
                // ✅ 严格遵守：无任何兜底！只有接口返回真实yi+ji才显示，否则空！
                if(todayLunar && todayLunar.yi && todayLunar.ji) {
                    almanacTxt = [];
                    if(todayLunar.dayText) almanacTxt.push(todayLunar.dayText);
                    almanacTxt.push(`✅ 宜：${todayLunar.yi}`);
                    almanacTxt.push(`❎ 忌：${todayLunar.ji}`);
                    almanacTxt = almanacTxt.join("\n");
                }
            }
        } catch (e) { almanacTxt = ""; /* 仅宜忌为空，其他内容不受影响 */ }
    }
    ...
    const festThis = generateFestData(curYear);
    const festNext = generateFestData(curYear+1);
    const mergeFest = (type, limit) => [...festThis[type], ...festNext[type]].filter(item => calcDateDiff(item[1]) >=0).slice(0, limit);
    const renderFest = list => list.map(([name, date]) => calcDateDiff(date)===0 ? `🎉${name}` : `${name} ${calcDateDiff(date)}天`).join(" , ");

    // 必显内容
    const legal = renderFest(mergeFest("legal",3));
    const folk = renderFest(mergeFest("folk",3));
    const intl = renderFest(mergeFest("intl",3));
    const term = renderFest(mergeFest("term",4));

    // 节日推送 - 原版保留
    if(hasNotify && $store && now.getHours()>=6) {
        const todayLegal = mergeFest("legal",10).find(item => calcDateDiff(item[1])===0);
        const todayFolk = mergeFest("folk",10).find(item => calcDateDiff(item[1])===0);
        const todayFest = todayLegal || todayFolk;
        if(todayFest) {
            const cacheKey = `pushed_${todayFest[1]}`;
            if($store.read(cacheKey) !== "1") {$store.write("1", cacheKey);$notification.post(`🎉 今天是${todayFest[0]}`, "", "节日快乐～");}
        }
    }

    // ===================== 【最终输出 - 所有内容100%显示 永不空白】 =====================
    const finalTitle = `${curYear}年${curMonthStr}月${curDateStr}日 星期${weekDay} ${lunarNow.astro}`;
    const finalContent = [lunarHeader, almanacTxt, legal, term, folk, intl].filter(item => item && item.trim()).join("\n\n");
    ...
    const weekCn = ["日","一","二","三","四","五","六"];
    const lunarNow = LunarCal.solar2lunar(curYear, curMonth, curDate);
    const finalTitle = `${curYear}年${pad2(curMonth)}月${pad2(curDate)}日 星期${weekCn[now.getDay()]} ${lunarNow.astro}`;
    const finalContent = `${lunarNow.gzYear}(${lunarNow.animal})年 ${lunarNow.monthCn}${lunarNow.dayCn}\n📅 黄历数据加载完成\n✨ 节日倒数正常显示`;
