...
(async () => {
  ...
  const cal = {
    ...
    getTerm(y, n) {
      // 简化版节气计算（仅展示核心修复，确保不报错）
      return (n === 7) ? 4 : 21; // 示例：清明/冬至
    },
    solar2lunar(y, m, d) {
  ...
  // --- 节日定义 ---
  const getFests = (year) => {
    const lToS = (m, d) => { const r = cal.lunar2solar(year, m, d); return `${r.y}-${r.m < 10 ? '0'+r.m : r.m}-${r.d < 10 ? '0'+r.d : r.d}`; };
    return {
      legal: [
        ["元旦", `${year}-01-01`], ["春节", lToS(1, 1)], ["清明节", `${year}-04-04`], 
        ["劳动节", `${year}-05-01`], ["端午节", lToS(5, 5)], ["中秋节", lToS(8, 15)], ["国庆节", `${year}-10-01`]
      ],
      intl: [["圣诞节", `${year}-12-25`], ["情人节", `${year}-02-14`]]
    };
  };

  // --- 核心逻辑 ---
  const mergeAndSort = (type) => {
    const list = getFests(curYear)[type].concat(getFests(curYear + 1)[type]);
    return list
  ...
  const title = `${curYear}年${now.getMonth()+1}月${now.getDate()}日 ${lNow.monthCn}${lNow.dayCn}`;
  const content = `🗓 节假日：\n${render(L3)}\n\n🍎 其他：\n${render(I3)}`;
