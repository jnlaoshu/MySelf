...
(async () => {
  ...
  // 工具函数【优化：新增空值过滤、增强补零、格式化】
  const pad2 = (n) => String(n).padStart(2, '0');
  const fmtYMD = (y, m, d) => `${y}-${pad2(m)}-${pad2(d)}`;
  const isEmpty = (val) => val === undefined || val === null || val === '' || val === '无';
  const filterEmpty = (arr) => arr.filter(item => !isEmpty(item)).join(' ');
  ...
  const cal = {
    ...
    solar2lunar(y, m, d) {
      ...
      return {
        ...
        gzMonth: this.toGanZhi((y-1900)*12 + m + 9 + (d >= this.getTerm(y, m*2-1)?1:0)), //修复偏移
        gzDay: this.toGanZhi(Date.UTC(y, m-1, 1)/86400000 + 25567 + 9 + d - 1), //修复偏移
  ...
  const getFests = (year) => {
    ...
    const weekDay = (m, n, w) => { 
      ...
      const maxDay = cal.solarDays(year, m);
      return fmtYMD(year, m, Math.min(day, maxDay)); //修复日期溢出
    };
  ...
  // 获取黄历详情【✅ 核心修复：解决宜/忌显示错误的所有问题】
  const almanacReq = getConfig('show_almanac', true) ? (async () => {
    const url = `https://raw.githubusercontent.com/zqzess/openApiData/main/calendar/${curYear}/${curYear}${pad2(curMonth)}.json`;
    const almanacList = await fetchJson(url, []);
    // 精准匹配当日黄历：年月+日期完全匹配
    const todayAlmanac = almanacList.find(item => 
      item.year == curYear && item.month == curMonth && item.day == curDate
    );
    // 基础信息保底
    const baseInfo = [
      `${lNow.gzYear}(${lNow.animal})年 ${lNow.gzMonth}月 ${lNow.gzDay}日`,
      lNow.term ? `▹ ${lNow.term}` : ''
    ].filter(Boolean).join(' ');

    // 无接口数据：只展示基础农历信息
    if (!todayAlmanac) return baseInfo;

    // 有接口数据：完整展示【宜/忌】+ 所有信息 ✅重点优化排版
    const suitText = isEmpty(todayAlmanac.suit) ? '诸事皆宜' : todayAlmanac.suit;
    const avoidText = isEmpty(todayAlmanac.avoid) ? '诸事无忌' : todayAlmanac.avoid;
    const extraDesc = filterEmpty([todayAlmanac.desc, todayAlmanac.value]);
    const almanacContent = [
      `${todayAlmanac.gzYear}年 ${todayAlmanac.gzMonth}月 ${todayAlmanac.gzDate}日 ${extraDesc}`,
      `✅ 宜：${suitText}`,
      `❎ 忌：${avoidText}`
    ].join('\n');
    return almanacContent;
  })() : Promise.resolve("");
  ...
  const merge = (k, count) => [...fThis[k], ...fNext[k]]
    .filter(i => dateDiff(i[1]) >= 0)
    .sort((a,b) => dateDiff(a[1]) - dateDiff(b[1])) //优化：按剩余天数排序
    .slice(0, count);
  ...
  const getTitle = () => {
    ...
    const diff = near ? dateDiff(near[1]) : 0;
    const defT = [
      `${curYear}年${pad2(curMonth)}月${pad2(curDate)}日 星期${"日一二三四五六"[now.getDay()]} ${lNow.astro}`,
      `{lunar}`
    ];
