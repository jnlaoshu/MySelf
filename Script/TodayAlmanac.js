/*
 * 今日黄历&节假日倒数 (V32.1 时间修正版)
 * ✅ 修复：重写时间计算逻辑 (UTC+8)，彻底解决 "日期不对应/跨天错误" 问题
 * ✅ 兼容：保留 V32 的精简结构 + GitHub 数据源
 * ✅ 匹配：增加日期格式容错 (2026-01-01 vs 2026-1-1)
 */
(async () => {
  ...
  const getBjDate = () => {
    // 1. 获取当前 UTC 时间戳
    const now = new Date();
    // 2. 加上 8 小时毫秒数 (8 * 60 * 60 * 1000 = 28800000)
    const bjTime = new Date(now.getTime() + (now.getTimezoneOffset() * 60000) + 28800000);
    return {
      y: bjTime.getFullYear(),
      m: bjTime.getMonth() + 1,
      d: bjTime.getDate(),
      day: bjTime.getDay()
    };
  ...
  // 匹配指纹 (同时匹配 "2026-01-16" 和 "2026-1-16")
  const MATCH = {
    s1: `${Y}-${P(M)}-${P(D)}`, // 标准补零
    s2: `${Y}-${M}-${D}`,       // 不补零
    d: D
  ...
  const getAlmanac = async () => {
    ...
    return new Promise(r => {
      ...
      return list.find(i => {
        // 优先匹配完整日期字符串
        if (i.date) {
          const dStr = String(i.date);
          if (dStr === MATCH.s1 || dStr === MATCH.s2) return true;
        }
        // 兜底匹配日 (防止跨月数据混淆，需确保 API 是按月返回的)
  ...
  try {
    ...
    $done({
      title: `${Y}年${P(M)}月${P(D)}日 星期${WEEK[N.day]} ${obj.astro}`,
