 ...
 * 更新：2026.01.15 修正优化版 - 完美解决宜/忌显示问题
 */
(async () => {
  /* ========== 配置与工具 ========== */
  ...
  // ✅ 修复核心BUG：日期差计算错误，修正偏移量，当日差值为0
  const dateDiff = (dateStr) => {
    const [y, m, d] = dateStr.split('-').map(Number);
    const target = new Date(y, m - 1, d).getTime();
    const today = new Date(curYear, now.getMonth(), now.getDate()).getTime();
  ...
  const cal = {
    ...
    solar2lunar(y, m, d) {
      ...
      // ✅ 修复BUG：干支年计算偏移量错误，year-4 → year-3
  ...
  // ✅ 核心修复：黄历「宜/忌」获取逻辑完全重构，精准读取当日数据，解决显示空白/错误
  const almanacReq = getConfig('show_almanac', true) ? (async () => {
    ...
    // 正确路径：直接取顶层 almanac 数组，匹配当日 day
    const almanacList = data?.almanac || [];
    const todayAlmanac = almanacList.find(item => Number(item.day) === curDay) || {};

    // 基础信息兜底
    const baseHead = `干支纪法：${lNow.gzYear}年 ${lNow.gzMonth}月 ${lNow.gzDay}日${lNow.term ? ` · ${lNow.term}` : ''}`;
    if (Object.keys(todayAlmanac).length === 0) return baseHead;

    // 宜/忌 数据处理 + 兜底为「无」
    const suitText = todayAlmanac.suit?.trim() || '无';
    const avoidText = todayAlmanac.avoid?.trim() || '无';
    const desc = [todayAlmanac.desc, todayAlmanac.value].filter(Boolean).join(" ");
    
    // 排版优化的黄历完整信息
    return `${todayAlmanac.gzYear}年 ${todayAlmanac.gzMonth}月 ${todayAlmanac.gzDate}日 ${desc}
✅ 宜：${suitText}
❎ 忌：${avoidText}`;
  })() : Promise.resolve("");
