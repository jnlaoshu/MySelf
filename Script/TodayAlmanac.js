 ...
 * 版本：纯精准无兜底版 | 只显示接口真实准确宜忌，无任何伪造兜底/默认值
 * 修复：初始化顺序+请求错误+字段精准匹配+移除所有虚假兜底
 */
(async () => {
  /* ========== 【置顶】工具函数 - 先定义后调用 杜绝初始化错误 ========== */
  ...
  // ✅ 终极精准匹配【接口真实字段】：字符串格式的纯数字 YYYYMMDD 无任何多余字符
  const todayDateKey = `${curYear}${padStart2(curMonth)}${padStart2(curDate)}`;
  ...
  const getLunarDesc = async (lunarData) => {
    ...
    // ✅ 终极精准匹配：接口的date是【字符串】YYYYMMDD，和todayDateKey完全一致
    const almanacItem = almanacList.find(item => item.date === todayDateKey);

    // 基础干支信息，无任何伪造
    const baseDesc = `${lunarData.gzYear}年 ${lunarData.gzMonth}月 ${lunarData.gzDay}日 ${lunarData.term || ""}`.trim();
    // ✅ 无兜底核心规则：匹配到接口真实数据 → 显示真实宜忌；匹配不到 → 只显示干支，绝不加假内容
    if (!almanacItem || !almanacItem.suit || !almanacItem.avoid) {
      return baseDesc;
    }
    // 只展示接口返回的【100%准确】的真实宜忌信息
  ...
  const generateTitle = () => {
    ...
    const nearFest = nearFests.sort((a,b)=>calcDateDiff(a[1])-calcDateDiff(b[1]))[0] || ["今日", todayDateKey];
    const diff = calcDateDiff(formatYmd(curYear, curMonth, curDate));
    const lunarDesc = `${lunarNow.gzYear}(${lunarNow.animal})年 ${lunarNow.monthCn}${lunarNow.dayCn}`;
    const solarDesc = `${curYear}年${padStart2(curMonth)}月${padStart2(curDate)}日（${lunarNow.astro}）`;
    ...
    if (titleMode === "random" || !$store) idx = Math.floor(Math.random() * titlePool.length);
    else {
      const cacheKey = `${TAG}_title_idx_${todayDateKey}`;
