/*
 * 今日黄历&节假日倒数（V6.0 终极完整版）
 * ✅ 恢复：四大类节日倒数 (法定/节气/民俗/国际) 分行显示
 * ✅ 恢复：特殊节日计算 (如母亲节、父亲节、感恩节)
 * ✅ 保持：GitHub Raw + UA 伪装 (解决网络拦截)
 * ✅ 保持：宽松日期匹配 (解决宜忌不显示)
 * ✅ 保持：时区校准 (解决日期偏差)
 */
(async () => {
  ...
  const fullDateStr = `${curYear}-${monthStr}-${todayDayStr}`;
  ...
  const getLunarDesc = async () => {
    ...
    if (!jsonData || !jsonData.days) return "⚠️ 黄历数据获取失败";

    // 🔍 暴力匹配逻辑 (兼容 16 和 "16" 和 "2024-01-16")
    const todayData = jsonData.days.find(item => {
      if (item.date && item.date === fullDateStr) return true;
      if (item.day && parseInt(item.day, 10) === curDate) return true;
      return false;
    });

    if (!todayData) return "📭 数据源暂无今日详情";

    const getVal = (...keys) => {
      for (const k of keys) if (todayData[k]) return todayData[k];
      return "";
    };
  ...
  const generateFestData = (year) => {
    ...
    const festData = {
      legal: [["元旦",formatYmd(year,1,1)],["春节",l2s(1,1)],["清明",formatYmd(year,4,qmDay)],["劳动",formatYmd(year,5,1)],["端午",l2s(5,5)],["中秋",l2s(8,15)],["国庆",formatYmd(year,10,1)]].filter(item => item[1]),
      folk: [["元宵",l2s(1,15)],["龙抬头",l2s(2,2)],["七夕",l2s(7,7)],["中元",l2s(7,15)],["重阳",l2s(9,9)],["腊八",l2s(12,8)],["小年",l2s(12,23)],["除夕",l2s(12,eve)]].filter(item => item[1]),
      intl: [["情人",formatYmd(year,2,14)],["妇女",formatYmd(year,3,8)],["母亲",weekSpecDay(5,2,0)],["儿童",formatYmd(year,6,1)],["父亲",weekSpecDay(6,3,0)],["万圣",formatYmd(year,10,31)],["感恩",weekSpecDay(11,4,4)],["平安",formatYmd(year,12,24)],["圣诞",formatYmd(year,12,25)]].filter(item => item[1]),
  ...
  const mergeFestList = (type, limit) => {
    ...
    return [...fThis, ...fNext]
      .filter(item => calcDateDiff(item[1]) >= -1) // 包含今天
      .slice(0, limit)
      .map(([name, date]) => {
