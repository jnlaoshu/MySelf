/*
 * 今日黄历&节假日倒数 (V32.2 逻辑修正版)
 * ✅ 修复：移除 parseInt 导致的日期匹配漂移问题 (解决获取到错误日期数据的Bug)
 * ✅ 增强：严格匹配 YYYY-MM-DD 或 YYYYMMDD 格式
 */
(async () => {
  // 1. 基础环境 (UTC+8)
  ...
  const MATCH = { 
    s1: `${Y}-${P(M)}-${P(D)}`, // 2026-01-17
    s2: `${Y}-${M}-${D}`,       // 2026-1-17 (兼容无补零)
    k: `${Y}${P(M)}${P(D)}`     // 20260117 (直接Key)
  };
  const WEEK = "日一二三四五六";
    ...
    return new Promise(r => {
      ...
      const scan = n => {
        ...
        if (n.yi || n.suit) {
            ...
            // 🔴 修复点：移除 parseInt(dStr) === D，防止匹配到仅仅是日期数字相同但月份年份不同的数据
            // 🟢 仅允许严格匹配完整日期字符串
            if (dStr.includes(MATCH.s1) || dStr.includes(MATCH.s2) || dStr === MATCH.k) {
                res = n;
            }
        }
