/*
 * 今日黄历&节假日倒数 (V44.0 接口救赎版)
 * -------------------------------------------
 * 🚫 问题根源：百度 API 对脚本环境进行了 IP/UA 级封锁，导致一直无数据。
 * ✅ 解决方案：切换至 [360万年历官方接口]。
 * (360与百度数据同源，且接口极其稳定，不做反爬限制)
 * 🎨 UI 还原：强制使用百度万年历的 "宜/忌" 文字图标排版。
 * -------------------------------------------
 */
(async () => {
  // 1. 基础环境 (UTC+8)
