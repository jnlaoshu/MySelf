/*
 * 今日黄历&节假日倒数（修复版）
 * ✅ 修复：将 GitHub 源替换为 jsDelivr CDN，解决宜忌不显示问题
 * ✅ 修复：优化日志输出，解决 Object 无法打印问题
 * ✅ 功能：农历/节气/法定假日/宜忌/星座/倒数日
 */
(async () => {
  // ========== 全局常量定义 & 调试开关 ==========
  const now = new Date();
  ...
  // 调试开关
  const DEBUG_MODE = false; // 日常使用建议设为 false
  ...
  const fetchJson = async (url) => {
    try {
      ...
      if (!Array.isArray(json.days)) {
        log("JSON格式校验失败：days不是数组");
        return { days: [] };
      }
      return json;
    } catch (e) {
      log(`JSON解析异常：${e.message}`);
      return { days: [] };
    }
  };
  ...
  const generateFestData = (year) => {
    ...
    const festData = {
      legal: [["元旦",formatYmd(year,1,1)],["春节",lunar2Solar(1,1)],["清明节",formatYmd(year,4,qmDay)],["劳动节",formatYmd(year,5,1)],["端午节",lunar2Solar(5,5)],["中秋节",lunar2Solar(8,15)],["国庆节",formatYmd(year,10,1)]].filter(item => item[1]),
      folk: [["元宵节",lunar2Solar(1,15)],["龙抬头",lunar2Solar(2,2)],["七夕节",lunar2Solar(7,7)],["中元节",lunar2Solar(7,15)],["重阳节",lunar2Solar(9,9)],["腊八节",lunar2Solar(12,8)],["小年",lunar2Solar(12,23)],["除夕",lunar2Solar(12,eve)]].filter(item => item[1]),
      intl: [["情人节",formatYmd(year,2,14)],["母亲节",weekSpecDay(5,2,0)],["父亲节",weekSpecDay(6,3,0)],["万圣节",formatYmd(year,10,31)],["平安夜",formatYmd(year,12,24)],["圣诞节",formatYmd(year,12,25)]].filter(item => item[1]),
  ...
  // ✅ 修复2：重写黄历获取逻辑，使用 jsDelivr CDN
  const getLunarDesc = async () => {
    if (!getConfig('show_almanac', true)) return "";
    
    // 关键修改：替换为高速 CDN 链接，避免 GitHub Raw 链接被墙或超时
    const apiUrl = `https://cdn.jsdelivr.net/gh/zqzess/openApiData@main/calendar_new/${curYear}/${curYear}${monthStr}.json`;
    
    const jsonData = await fetchJson(apiUrl);
    const dayList = jsonData.days || [];

    // 宽松匹配：兼容 API 返回的 day 是数字或字符串的情况
    const todayData = dayList.find(item => {
      const apiDayNum = Number(item.day);
      return apiDayNum === curDate;
    });
  ...
  try {
    ...
    console.log(`❌ 运行异常: ${mainErr.message}`);
    const y = curYear, m = monthStr, d = todayDayStr;
    $done({ title: `${y}年${m}月${d}日`, content: "配置有误或网络超时", icon: "calendar", "icon-color": "#FF9800" });
  }
})().catch(error => {
  console.log(`❌ 致命错误: ${error}`);
  $done({ title: "脚本错误", content: "请检查日志", icon: "calendar", "icon-color": "#FF9800" });
});
