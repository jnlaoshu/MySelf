...
(async () => {
  ...
  const fetchJson = async (url, fallback) => {
    ...
    try { const res = await httpGet(url); return res ? JSON.parse(res) : fallback; } 
    catch (e) { console.log(`JSON解析失败:${e.message}`); return fallback; }
  };
  // 天数差计算-修复边界值
  const dateDiff = (dateStr) => {
    const [y, m, d] = dateStr.split('-').map(Number);
    const target = new Date(y, m - 1, d).setHours(0,0,0,0);
    const current = new Date(curYear, curMonth - 1, curDate).setHours(0,0,0,0);
  ...
  const cal = {
    ...
    solar2lunar(y, m, d) {
      let i, leap=0, temp=0;let offset=(Date.UTC(y,m-1,d)-Date.UTC(1900,0,31))/86400000;
      for(i=1900;i<2101&&offset>0;i++){temp=this.lYearDays(i);offset-=temp;}
      if(offset<0){offset+=temp;i--;}const year=i;let isLeap=false;leap=this.leapMonth(i);
      for(i=1;i<13&&offset>0;i++){if(leap>0&&i===(leap+1)&&!isLeap){--i;isLeap=true;temp=this.leapDays(year);}else{temp=this.monthDays(year,i);}if(isLeap&&i===(leap+1))isLeap=false;offset-=temp;}
      if(offset===0&&leap>0&&i===leap+1){if(isLeap)isLeap=false;else{isLeap=true;--i;}}
      if(offset<0){offset+=temp;i--;}
      const month=i, day=offset+1;const gzY=this.toGanZhi(year-4);
  ...
  const checkNotify = (list) => {
    ...
    if (todayFest && now.getHours() >=6 && $store) {
