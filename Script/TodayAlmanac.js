...
(async () => {
  ...
  // ✅ 修复核心1：统一今日日期格式为标准补零格式，和接口数据完全一致
  ...
  const getLunarDesc = async (lunarData) => {
    ...
    if (rawData) {
       if (Array.isArray(rawData.almanac)) almanacList = rawData.almanac;
       else if (Array.isArray(rawData)) almanacList = rawData;
    }

    // ✅ 修复核心2：移除错误的 day 纯数字匹配，只保留【精准的完整日期匹配】，杜绝跨月错误
    const almanacItem = almanacList.find(item => {
      if (!item.date) return false;
      const dateVal = String(item.date).trim();
      // 兼容 2026-01-16 和 20260116 两种格式
      const normDate = dateVal.includes('-') ? dateVal : `${dateVal.slice(0,4)}-${dateVal.slice(4,6)}-${dateVal.slice(6,8)}`;
