...
(async () => {
  ...
  const curDayPad = String(curDay).padStart(2, '0'); // ✅ 关键：转补零字符串，和源数据日期格式完全一致
  ...
  // ✅ 增强请求稳定性 超时10秒，失败返回null，不解析错误数据
  ...
  const almanacReq = getConfig('show_almanac', true) ? (async () => {
    ...
    // 获取源数据：根节点就是纯数组，无任何嵌套
    const sourceData = await fetchJson(targetUrl, []);

    // 1. 精准匹配今日数据：源数据day是补零字符串，用curDayPad严格匹配，无任何容错
    const todayRealData = Array.isArray(sourceData) 
      ? sourceData.find(item => item.day === curDayPad) 
      : null;

    // 2. 纯读网站真实数据，无任何兜底假值
    if (todayRealData && todayRealData.yi && todayRealData.ji) {
      // 显示的内容 = 网站源数据的 yi / ji，100%精准一致
  ...
  $done({title:"黄历加载异常",content:e.message,icon:"exclamationmark.triangle"});
});
