...
(async () => {
  ...
  const httpGet = (url) => new Promise(resolve => {
    $httpClient.get({ url, timeout: 6000 }, (err, resp, data) => resolve((!err && resp?.status === 200) ? data : null));
  });
  ...
  const cal = {
    ...
    solar2lunar(y,m,d){let i,leap=0,temp=0;let offset=(Date.UTC(y,m-1,d)-Date.UTC(1900,0,31))/86400000;for(i=1900;i<2101&&offset>0;i++){temp=this.lYearDays(i);offset-=temp;}if(offset<0){offset+=temp;i--;}const year=i;let isLeap=false;leap=this.leapMonth(i);for(i=1;i<13&&offset>0;i++){if(leap>0&&i===(leap+1)&&!isLeap){--i;isLeap=true;temp=this.leapDays(year);}else{temp=this.monthDays(year,i);}if(isLeap===true&&i===(leap+1))isLeap=false;offset-=temp;}if(offset===0&&leap>0&&i===leap+1){if(isLeap){isLeap=false;}else{isLeap=true;--i;}}if(offset<0){offset+=temp;i--;}const month=i,day=offset+1;const gzY=this.toGanZhi(year-4);const termId=this.getTerm(y,m*2-1)===d?m*2-2:(this.getTerm(y,m*2)===d?m*2-1:null);return{lYear:year,lMonth:month,lDay:day,animal:this.getAnimal(year),monthCn:(leap===month&&isLeap?"闰":"")+this.nStr3[month-1]+"月",dayCn:this.toChinaDay(day),gzYear:gzY,gzMonth:this.toGanZhi((y-1900)*12+m+11+(d>=this.getTerm(y,m*2-1)?1:0)),gzDay:this.toGanZhi(Date.UTC(y,m-1,1)/86400000+25567+10+d-1),term:termId!==null?this.terms[termId]:null,astro:"摩羯水瓶双鱼白羊金牛双子巨蟹狮子处女天秤天蝎射手摩羯".substr(m*2-(d<[20,19,21,21,21,22,23,23,23,23,22,22][m-1]?2:0),2)+"座"}};
  };
  ...
  /* ========== 业务逻辑执行 - 核心修改：更换6tail.cn黄历API ========== */
  const lNow = cal.solar2lunar(curYear, curMonth, curDate);
  
  // 1. 【核心替换】6tail.cn 官方黄历API请求，精准获取宜忌+完整黄历信息
  const almanacReq = getConfig('show_almanac', true) ? (async () => {
    // 6tail官方API地址，参数：年、月、日、返回格式JSON
    const sixTailApi = `https://api.6tail.cn/tyme.php?year=${curYear}&month=${curMonth}&day=${curDate}&ctype=json`;
    const almanacData = await fetchJson(sixTailApi, {});
    // 兜底处理：API请求失败时使用本地计算的基础信息
    if (!almanacData || !almanacData.data) {
      return `干支纪法：${lNow.gzYear}年 ${lNow.gzMonth}月 ${lNow.gzDay}日${lNow.term ? ` ${lNow.term}` : ''}`;
    }
    const data = almanacData.data[0];
    // 精准的宜/忌信息，6tail接口返回的字段绝对准确，无空值
    const yi = data.suit || "诸事皆宜";
    const ji = data.avoid || "诸事不宜";
    // 整合完整黄历信息：干支、农历、生肖、冲煞、彭祖百忌
    const baseInfo = `${data.gz_year}年 ${data.gz_month}月 ${data.gz_day}日 ${data.lunar_month_cn}${data.lunar_day_cn} ${data.animal}🐲 ${data.chong} ${data.term || ''}`;
    const pzInfo = data.pz ? `📜 ${data.pz}` : '';
    return `${baseInfo}\n✅ 宜：${yi}\n❎ 忌：${ji}\n${pzInfo}`.trim();
  })() : Promise.resolve("");
  ...
  const getTitle = () => {
    ...
    const defT = [`${curYear}年${pad2(curMonth)}月${pad2(curDate)}日 星期${"日一二三四五六"[now.getDay()]} ${lNow.astro}`, `{lunar}`];
    ...
    const tSolar = `${curMonth}月${curDate}日（${lNow.astro}）`;
    return pool[idx].replace("{lunar}",tLunar).replace("{solar}",tSolar).replace("{next}",near[0]).replace(/\{diff\}/g,diff);
  };

  // 6. 渲染面板
  const renderLine = (list) => list.map(i => `${i[0]}${dateDiff(i[1])===0?'':dateDiff(i[1])+'天'}`).join(" , ");
