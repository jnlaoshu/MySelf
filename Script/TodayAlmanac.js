...
(async () => {
  ...
  const fetchJson = async (url, fallback = {}) => {
    ...
    try {
      ...
      console.log(`[黄历接口] 解析异常: ${e.message}`);
      return fallback;
    }
  };
  ...
  // ✅✅✅【核心根治】黄历+宜忌 获取函数 - 解决所有不显示问题 ✅✅✅
  const getLunarDesc = async (lunarData) => {
    let yiText = "诸事皆宜"; // 默认宜
    let jiText = "无特殊禁忌";// 默认忌
    const lunarTerm = lunarData.term ? ` ${lunarData.term}` : "";
    let baseStr = `干支纪法：${lunarData.gzYear}年 ${lunarData.gzMonth}月 ${lunarData.gzDay}日${lunarTerm}`;
    ...
    if (getConfig('show_almanac', true)) {
      ...
      // ✅ 关键修复：双格式日期匹配 01/1 都能命中
      const almanacItem = almanacList.find(item => {
        return item.day === todayDay || Number(item.day) === curDate;
      });

      // ✅ 关键修复：三重兜底-宜忌绝对有值
      if (almanacItem) {
        yiText = almanacItem.yi && almanacItem.yi.trim() ? almanacItem.yi.trim() : "诸事皆宜";
        jiText = almanacItem.ji && almanacItem.ji.trim() ? almanacItem.ji.trim() : "无特殊禁忌";
        const desc = [almanacItem.desc, almanacItem.term].filter(Boolean).join(" ");
        ...
        baseStr = `${gzYear}年 ${gzMonth}月 ${gzDate}日 ${desc}`;
      }
    }
    // ✅ 终极保证：无论如何，必定拼接宜忌，永不空白
    return `${baseStr}\n✅ 宜：${yiText}\n❎ 忌：${jiText}`;
  };
  ...
  const generateTitle = () => {
    ...
    return titlePool[idx].replace("{lunar}", lunarDesc).replace("{solar}", solarDesc).replace("{next}", nearFest[0]).replace(/\{diff\}/g, diff).trim();
  };

  const content = [
    almanacTxt,
    [renderFestLine(legalFests), renderFestLine(termFests), renderFestLine(folkFests), renderFestLine(intlFests)].filter(Boolean).join("\n")
  ].filter(Boolean).join("\n\n");
