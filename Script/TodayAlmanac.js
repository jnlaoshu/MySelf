/*
 * 今日黄历&节假日倒数（回滚修复版 - 专注日期格式修正）
 * ✅ 修复核心：重写日期匹配逻辑，解决 JSON 中 "01" 与 1 不匹配的问题
 * ✅ 修复时区：强制锁定 Asia/Shanghai，防止脚本运行环境导致日期偏差
 * ✅ 修复兼容：同时支持 day 字段和 date 字段 (YYYY-MM-DD) 匹配
 * ✅ 保留功能：Yi/yi 大小写全兼容
 */
(async () => {
  // ========== 全局常量定义 & 调试开关 ==========
  // 🕒 修复1：强制将当前时间转换为北京时间，解决时区导致的日期错位
  const getBjDate = () => {
    const d = new Date();
    // 将 UTC 时间转换为北京时间 (UTC+8) 的 Date 对象
  ...
  const curDate = now.getDate(); // 获取北京时间的“日”
  ...
  const fetchJson = async (url) => {
    try {
      ...
      if (!Array.isArray(json.days)) return { days: [] };
      return json;
    } catch (e) {
      log(`解析失败：${e.message}`);
      return { days: [] };
  ...
  // ✅ 修复2：重写 getLunarDesc，彻底解决日期匹配不到的问题
  const getLunarDesc = async () => {
    ...
    // 🔍 核心修复：日期匹配逻辑升级
    // 1. 优先尝试 Number 匹配 (忽略 "01" 与 1 的区别)
    // 2. 其次尝试完整 date 字符串匹配 (防止 day 字段不存在)
    const todayData = dayList.find(item => {
      // 匹配方式A: day 字段转为数字后是否相等 (推荐)
      if (item.day !== undefined && Number(item.day) === curDate) return true;
      
      // 匹配方式B: 如果有 date 字段 (如 "2024-01-16")，尝试解析匹配
      if (item.date) {
        const parts = item.date.split('-');
        if (parts.length === 3 && Number(parts[2]) === curDate) return true;
      }
      
      // 匹配方式C: 字符串严格匹配 (兜底)
      return String(item.day).padStart(2, '0') === todayDayStr;
    });

    if (!todayData) {
      log(`未找到日期 ${todayDayStr} 的数据`);
      return "";
    }
  ...
  $done({ title: "脚本运行错误", content: "请检查日志", icon: "exclamationmark.triangle", "icon-color": "#FF0000" });
});
