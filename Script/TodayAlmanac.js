 ...
 * 更新：2026.01.15 优化版 (Fix: GanZhi calculation & Almanac parsing)
 */
(async () => {
  /* ========== 常量配置 & 环境初始化 ========== */
  ...
  const getLunarDesc = async (lunarData) => {
    ...
    // Fix: Match by full date string (yyyy-mm-dd) instead of simple day number
    const targetYmd = formatYmd(curYear, curMonth, curDate); // 2025-01-15
    let almanacItem = almanacList.find(i => {
        if (i.date === targetYmd) return true;
        // Handle 2025-1-15 format or other variations
        if (i.date) {
            const p = i.date.split(/[-/]/).map(Number);
            return p.length === 3 && p[0] === curYear && p[1] === curMonth && p[2] === curDate;
        }
        // Fallback to day property
        if (i.day && Number(i.day) === curDate) return true;
        return false;
    });

    // Fallback: If no date match found, try index-based if consistent
    if (!almanacItem && almanacList.length >= curDate) {
        const fallback = almanacList[curDate - 1];
        if (fallback) {
             let seemsCorrect = true;
             // Verify day matches if present
             if (fallback.day && Number(fallback.day) !== curDate) seemsCorrect = false;
             if (seemsCorrect) almanacItem = fallback;
        }
    }
