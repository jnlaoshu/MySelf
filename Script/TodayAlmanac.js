...
(async () => {
  ...
  const getFests = (year) => {
    ...
    const eveDays = cal.monthDays(year, 12); // 除夕可能是29也可能是30
    ...
    return {
      ...
      folk: [
        ["除夕", lToS(12, eveDays)], ["元宵节", lToS(1, 15)], ["龙抬头", lToS(2, 2)],
  ...
  const almanacReq = getConfig('show_almanac', true) ? (async () => {
    ...
    return `${item.gzYear}年 ${item.gzMonth}月 ${item.gzDate}日 ${[item.desc, item.term, item.value].filter(Boolean).join(" ")}\n✅ 宜：${item.suit || '——'}\n❎ 忌：${item.avoid || '——'}`;
  })() : Promise.resolve("");
  ...
  const merge = (k, count) => {
    return [...fThis[k], ...fNext[k]]
      .filter(i => dateDiff(i[1]) >= 0) // 只保留今天及以后的
      .sort((a, b) => new Date(a[1]) - new Date(b[1])) // 关键：按日期升序排列
      .slice(0, count);
  };
  ...
  const getTitle = () => {
    ...
    const pool = (Array.isArray(titles) && titles.length) ? titles : [`${curYear}年${pad2(now.getMonth()+1)}月${pad2(now.getDate())}日 星期${"日一二三四五六"[now.getDay()]} {lunar}`];
    let idx = Math.floor(Math.random() * pool.length);
    const tLunar = `${lNow.gzYear}(${lNow.animal})年 ${lNow.monthCn}${lNow.dayCn}`;
    return pool[idx].replace("{lunar}", tLunar).replace("{next}", near[0]).replace(/\{diff\}/g, diff);
  };

  const renderLine = (list) => list.map(i => {
    const d = dateDiff(i[1]);
    return `${i[0]}${d === 0 ? '[今日]' : d + '天'}`;
  }).join(" , ");
  ...
  $done({ title: "脚本运行错误", content: e.message });
});
