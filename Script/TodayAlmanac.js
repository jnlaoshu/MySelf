/*
 * 今日黄历&节假日倒数 (原版标题轮播逻辑恢复)
 * 修复：标题副标题分行、成都特定日期、黄历宜忌、农历算法、星座
 */
...
(async () => {
  ...
  const cal = {
    ...
    solar2lunar(y, m, d) {
      ...
      if (offset < 0) { offset += temp; i--; }
      return {
        lYear: year, animal: this.Animals[(year - 4) % 12],
  ...
  const getFests = (year) => {
    ...
    return {
      legal: [
        ...
        ["春季开学", `2026-03-02`], ["成都春假", fmtYMD(year, 4, qmDay + 1)],
        ["劳动节", `${year}-05-01`], ["端午", lToS(5, 5)], ["全国高考", `${year}-06-07`],
        ["暑假", `${year}-07-05`], ["中秋", lToS(8, 15)], ["国庆", `${year}-10-01`],
        ["成都秋假", getAutumnBreak(year)]
      ],
      folk: [["除夕", lToS(12, 30)], ["元宵", lToS(1, 15)], ["腊八", lToS(12, 8)]],
  ...
  // 恢复分行逻辑：第一行 Title，第二行 Subtitle
  const title = `${curYear}年${curMonth}月${curDay}日 星期${"日一二三四五六"[now.getDay()]} ${lNow.animal}年`;
  const subtitle = `${lNow.monthCn}${lNow.dayCn} ${lNow.gzYear}年 ${getAstro(curMonth, curDay)}`;
