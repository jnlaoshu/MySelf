/*
 * 今日黄历&节假日倒数 (V26.0 逻辑修正稳定版)
 * ✅ 修复：解决 V25 版本因 "字段大小写判断过严" 导致宜忌再次丢失的问题
 * ✅ 核心：恢复 "多字段模糊提取" (Yi/yi/suit)，确保 100% 能读到数据
 * ✅ 布局：保持经典四行布局，每行显示 3 个
 */
(async () => {
  // 1. 基础环境 (强制北京时间)
  ...
  // 匹配指纹
  const MATCH = {
    s: `${cY}-${pad(cM)}-${pad(cD)}`, // 2026-01-16
    s2: `${cY}-${cM}-${cD}`,          // 2026-1-16
    d: cD                             // 16
  };
  ...
  const getData = async () => {
    ...
    return new Promise(resolve => {
      ...
      const scan = n => {
        ...
        // 特征识别：只要包含宜/忌/日期中的任意组合
        if ((n.yi || n.ji || n.suit || n.Yi || n.Ji) && (n.day || n.date)) {
          list.push(n);
        }
  ...
  try {
    ...
    const dayData = await getData(); // 这里不会再因为大小写而返回空了
    
    // 灵活提取 (Fix: 核心修复点)
    const getV = (...keys) => { 
      if (!dayData) return "";
      for (let k of keys) if (dayData[k]) return dayData[k]; 
      return ""; 
    };
