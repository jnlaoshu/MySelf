...
(async () => {
    ...
    solar2lunar(y, m, d) {
      ...
      for(i = 1; i < 13 && offset > 0; i++){
        if(leap > 0 && i === (leap + 1) && !isLeap){ 
          ...
          temp = this.monthDays(year, isLeap ? 0 : i); 
        }
      ...
      // 修复干支纪年计算
      const gzY = this.toGanZhi(year - 1900 + 36);  // 1900年为庚子年
      
      // 修复干支纪月计算
      const gzMonthNum = (year - 1900) * 12 + m + 12;
      const gzM = this.toGanZhi(gzMonthNum);
      
      // 修复干支纪日计算
      const baseTime = Date.UTC(1900, 0, 31) / 86400000;  // 1900年1月31日为基准
      const targetTime = Date.UTC(y, m-1, d) / 86400000;
      const gzDayNum = Math.floor(targetTime - baseTime + 60) % 60;  // 1900年1月31日为甲午日
      const gzD = this.toGanZhi(gzDayNum);
      
      const term1Day = this.getTerm(y, m*2-1);
      const term2Day = this.getTerm(y, m*2);
      const termId = term1Day === d ? m*2-2 : (term2Day === d ? m*2-1 : null);
  ...
  const generateFestData = (year) => {
    ...
    return {
      legal: [
        ...
        ["寒假", formatYmd(year, 1, 31)],
        ["春节", lunar2Solar(1, 1)],
        ["开学", formatYmd(year, 3, 2)],
        ["清明节", formatYmd(year, 4, qmDay)],
        ["春假", formatYmd(year, 4, qmDay+1)],
        ["劳动节", formatYmd(year, 5, 1)],
        ["端午节", lunar2Solar(5, 5)],
        ["高考", formatYmd(year, 6, 7)],
        ["暑假", formatYmd(year, 7, 4)],
        ["中秋节", lunar2Solar(8, 15)],
        ["国庆节", formatYmd(year, 10, 1)],
        ["秋假", weekSpecDay(11, 2, 3)]
      ],
      folk: [
        ["元宵节", lunar2Solar(1, 15)],
        ["龙抬头", lunar2Solar(2, 2)],
        ["七夕节", lunar2Solar(7, 7)],
        ["中元节", lunar2Solar(7, 15)],
        ["重阳节", lunar2Solar(9, 9)],
        ["寒衣节", lunar2Solar(10, 1)],
        ["下元节", lunar2Solar(10, 15)],
        ["腊八节", lunar2Solar(12, 8)],
        ["北方小年", lunar2Solar(12, 23)],
        ["南方小年", lunar2Solar(12, 24)],
      ...
      intl: [
        ...
        ["感恩节", weekSpecDay(11, 4, 4)]
      ],
      term: Array.from({length: 24}, (_, i) => { 
        const m = Math.floor(i/2) + 1, id = i + 1; 
        return [LunarCal.terms[i], formatYmd(year, m, LunarCal.getTerm(year, id))]; 
      })
    };
  };

  /* ========== 黄历信息获取（修复） ========== */
  const getAlmanacData = async () => {
    ...
    try {
      ...
      const response = await fetchJson(url, null);
      
      if (!response?.data?.[0]?.almanac) return null;
      
      // 找到当前日期的黄历信息
      const almanac = response.data[0].almanac.find(item => Number(item.day) === curDate);
      ...
      return {
        ...
        term: almanac.term || "",
        value: almanac.value || "",
        suit: almanac.suit || "诸事皆宜",
        avoid: almanac.avoid || "无特殊禁忌"
      };
    } catch (error) {
  ...
  /* ========== 主业务逻辑执行 ========== */
  const [lunarNow, almanacData, titles, blessMap] = await Promise.all([
  ...
  const generateAlmanacText = () => {
    ...
    const baseDesc = `干支纪法：${lunarNow.gzYear}年 ${lunarNow.gzMonth}月 ${lunarNow.gzDay}日`;
    const termPart = lunarNow.term ? `· ${lunarNow.term}` : "";
    
    if (!almanacData) {
      return `${baseDesc}${termPart}`.trim();
    }
    
    const almanacDesc = [almanacData.desc, almanacData.term, almanacData.value]
      .filter(Boolean)
      .join(" ") || "传统吉日";
    
    return `${baseDesc}${termPart}\n📅 ${almanacDesc}\n✅ 宜：${almanacData.suit}\n❌ 忌：${almanacData.avoid}`;
  };
