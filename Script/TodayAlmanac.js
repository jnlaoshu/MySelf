...
(async () => {
  ...
  const cal = {
    ...
    getAnimal(y) { return "鼠牛虎兔龙蛇马羊猴鸡狗猪"[(y-4)%12]; },
    solar2lunar(y, m, d) {
      let i, leap = 0, temp = 0;
      ...
      return {
        ...
        term: this.getTerm(y, m*2-1) === d ? this.terms[m*2-2] : (this.getTerm(y, m*2) === d ? this.terms[m*2-1] : null),
  ...
  const getFests = (year) => {
    ...
    const lastDay12 = cal.monthDays(year, 12); // 除夕为腊月最后一天
    ...
    return {
      ...
      folk: [
        ["除夕", lToS(12, lastDay12)], ["元宵节", lToS(1, 15)], ["龙抬头", lToS(2, 2)],
  ...
  const almanacReq = getConfig('show_almanac', true) ? (async () => {
    ...
    const baseHead = `干支纪法：${lNow.gzYear}年 ${lNow.monthCn}${lNow.dayCn}`;
    if (!item) return baseHead + (lNow.term ? ` ${lNow.term}` : '');
    return `${item.gzYear}年 ${item.gzMonth}月 ${item.gzDate}日 ${item.term || ''}\n✅ 宜：${item.suit || '——'}\n❎ 忌：${item.avoid || '——'}`;
  })() : Promise.resolve("");
  ...
  const merge = (k, count) => {
    return [...fThis[k], ...fNext[k]]
      ...
      .filter(i => i.diff >= 0)
      .sort((a, b) => a.diff - b.diff) // 按时间先后排序
      .slice(0, count)
      .map(i => [i.name, i.date]);
  };
  ...
  const getTitle = () => {
    ...
    return pool[idx].replace("{lunar}", tLunar).replace("{next}", near[0]).replace(/\{diff\}/g, dateDiff(near[1]));
  };

  const renderLine = (list) => list.map(i => {
    const d = dateDiff(i[1]);
    return `${i[0]}${d === 0 ? '[今天]' : d + '天'}`;
  }).join(" , ");
