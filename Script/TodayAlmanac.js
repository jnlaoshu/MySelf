...
(async () => {
  ...
  const cal = {
    ...
    toChinaDay(d) {
      ...
      if (d === 20) return "二十";
      if (d === 30) return "三十";
      const s = this.nStr2[Math.floor(d / 10)];
      const g = this.nStr1[d % 10];
      return s + g;
    },
    ...
    solar2lunar(y, m, d) {
      ...
      for (i = 1; i < 13 && offset > 0; i++) {
        ...
        offset -= temp;
        if (isLeap && i === (leap + 1) && offset < 0) {
           // 回退逻辑，如果减过头了
           // (Simple implementation assumes linear subtraction is fine, typically this loop structure is standard)
        }
      ...
      for (i = 1; i < 13 && offset > 0; i++) {
        if (leap > 0 && i === (leap + 1) && !isLeapMonth) {
          --i;
          isLeapMonth = true;
          temp = this.leapDays(iYear);
        } else {
          temp = this.monthDays(iYear, i);
        }
      ...
      return {
        ...
        monthCn: (isLeapMonth ? "闰" : "") + this.nStr3[month - 1] + "月",
  ...
  const getTitle = () => {
    ...
    return pool[idx]
      .replace("{lunar}", tLunar)
      .replace("{solar}", tSolar)
      .replace("{next}", near[0])
      .replace(/\{diff\}/g, diff);
  };
