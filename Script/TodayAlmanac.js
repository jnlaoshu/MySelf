 ...
 * 更新：2026.01.15 优化版 | 修复宜忌显示+删除干支+迁移农历至标题
 */
...
(async () => {
  ...
  const almanacReq = getConfig('show_almanac', true) ? (async () => {
    ...
    // 修复：原解析路径错误，正确路径为 data.almanac 直接查找当日数据
    const item = data?.almanac?.find(i => Number(i.day) === now.getDate());
    if (!item) return lNow.term ? `${lNow.term}` : '';
    // 规范宜忌显示，无内容时显示「无」，排版整洁
    const suitText = item.suit && item.suit.trim() ? item.suit.trim() : "无";
    const avoidText = item.avoid && item.avoid.trim() ? item.avoid.trim() : "无";
    const termText = item.term ? `${item.term} ` : '';
    return `${termText}\n✅ 宜：${suitText}\n❎ 忌：${avoidText}`;
  })() : Promise.resolve("");
  ...
  // ✅ 核心调整：农历信息全部移入标题栏轮换显示 + 删除干支
  const getTitle = () => {
    const near = [L3[0], F3[0], I3[0]].sort((a,b) => dateDiff(a[1]) - dateDiff(b[1]))[0];
    const diff = dateDiff(near[1]);
    // 标题栏集成【纯农历信息】：农历XX月XX日，无任何干支/生肖
    const pureLunar = `农历${lNow.monthCn}${lNow.dayCn}`;
    const defT = [
      `${curYear}年${pad2(now.getMonth()+1)}月${pad2(now.getDate())}日 星期${"日一二三四五六"[now.getDay()]} ${pureLunar}`,
      `${pureLunar} ${lNow.astro} | 距离${near[0]}还有${diff}天`
    ];
