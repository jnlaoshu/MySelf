/*
 * 今日黄历&节假日倒数（V5.2 最终修复版）
 * ✅ 修复：补全缺失的 formatYmd 函数，解决 "Can't find variable" 报错
 * ✅ 保持：GitHub Raw + User-Agent 伪装 (防止拦截)
 * ✅ 保持：宽松日期匹配 (解决 01 vs 1 不匹配问题)
 * ✅ 保持：完整农历算法 (解决 lYearDays 报错)
 */
(async () => {
  // ========== 1. 环境与时间 ==========
  const getBjDate = () => {
  ...
  try {
    ...
    const finalContent = [lunarHeader, almanacTxt, festTxt].filter(Boolean).join("\n\n");
