...
(async () => {
  ...
  const mergeAndSort = (key) => {
    const list = getFests(curYear)[key].concat(getFests(curYear + 1)[key]);
    return list
  ...
  let almanacTxt = `干支纪年：${lNow.gzYear}年（${lNow.animal}）`;
  ...
  if (alData) {
    try {
      ...
      const item = json.data[0].almanac.find(i => Number(i.day) === curDay);
      if (item) {
        almanacTxt = `✅ 宜：${item.suit}\n❎ 忌：${item.avoid}`;
      }
    } catch(e) {}
  }

  const L3 = mergeAndSort("legal");
  const T3 = mergeAndSort("term");
  const F3 = mergeAndSort("folk");

  const render = (arr) => arr.map(i => `${i[0]}${dateDiff(i[1]) === 0 ? "今天" : dateDiff(i[1]) + "天"}`).join(" , ");

  const title = `${curYear}年${curMonth}月${curDay}日 星期${"日一二三四五六"[now.getDay()]} ${lNow.monthCn}${lNow.dayCn}`;
