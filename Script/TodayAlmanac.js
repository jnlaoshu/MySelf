 ...
 * 今日黄历&节假日倒数
 * 1. 恢复原版【标题栏轮播】排版
 * 2. 包含成都义教 2026-03-02 开学、全国高考 06-07
 * 3. 修正民俗/节假日跨年排序显示错误
 */
...
(async () => {
  ...
  const merge = (key) => {
    return [].concat(getFests(curYear)[key], getFests(curYear + 1)[key])
      ...
      .sort((a, b) => {
        return new Date(a[1].replace(/-/g, '/')).getTime() - new Date(b[1].replace(/-/g, '/')).getTime();
      })
      .slice(0, 5);
  };
