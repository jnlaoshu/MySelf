...
(async () => {
  ...
  const cal = {
    ...
    // 基础黄历数据（用于API失败时本地计算）
    getBasicAlmanac(y, m, d) {
      const dayOfYear = Math.floor((new Date(y, m-1, d) - new Date(y, 0, 0)) / 86400000);
      const seed = (y * 10000 + m * 100 + d) % 1000;
      
      // 基于日期生成一些基础的宜忌（这是简化的方法）
      const yiList = ["祭祀", "祈福", "求嗣", "开光", "出行", "解除", "伐木", "拆卸", "进人口", "安床", "动土", "起基", "上梁", "立券", "交易", "纳财", "造仓", "开池", "栽种", "纳畜", "破土", "安葬", "入殓", "立碑", "谢土"];
      const jiList = ["嫁娶", "开市", "立券", "纳财", "分居", "开仓", "出货财", "动土", "破土", "掘井", "开光", "祈福", "求嗣", "出行", "解除", "纳采", "问名", "嫁娶", "开市", "立券", "纳财", "造庙", "造桥", "造船", "作灶", "造畜椆栖"];
      ...
      for (let i = 0; i < 6; i++) {
        yi.push(yiList[(seed + i * 3) % yiList.length]);
        ji.push(jiList[(seed + i * 5 + 13) % jiList.length]);
      }
      
      return {
        suit: [...new Set(yi)].slice(0, 6).join("、"),
        avoid: [...new Set(ji)].slice(0, 6).join("、")
      };
    },
    
    // 获取年份对应的天干地支索引（以1900年为庚子年基准）
    getYearGanZhiIndex(y) {
      return (y - 1900) % 60;
    },
    
    // 获取月份对应的天干地支索引
    // 月干支：以年干为基础，根据五虎遁口诀计算
    // 甲己之年丙作首，乙庚之岁戊为头
    // 丙辛必定寻庚起，丁壬壬位顺行流
    // 戊癸甲寅为正月，干支从此定根由
    getMonthGanZhiIndex(y, m) {
      const yearGan = this.getYearGanZhiIndex(y) % 10; // 获取年干
      const monthOffset = [2, 4, 6, 8, 0]; // 五虎遁：甲己=2(丙), 乙庚=4(戊), 丙辛=6(庚), 丁壬=8(壬), 戊癸=0(甲)
      const ganIndex = (monthOffset[Math.floor(yearGan / 2)] + m - 1) % 10;
      const zhiIndex = (m + 1) % 12; // 正月从寅开始
 ...
 // 民俗节日
  ...
  const almanacReq = getConfig('show_almanac', true) ? (async () => {
    // 首先使用本地计算的黄历数据作为基础
    const basicAlmanac = cal.getBasicAlmanac(curYear, now.getMonth() + 1, now.getDate());
    ...
    try {
      ...
      // 检查数据结构是否正确
      if (!data || !data.data || !Array.isArray(data.data) || !data.data[0] || !data.data[0].almanac) {
      ...
      if (item) {
        ...
        return `${item.gzYear}年 ${item.gzMonth}月 ${item.gzDate}日 ${desc}\n✅ 宜：${item.suit || basicAlmanac.suit}\n❎ 忌：${item.avoid || basicAlmanac.avoid}`;
      }
      
    } catch (error) {
      // API请求失败，使用本地计算的数据
      console.log(`API获取失败，使用本地数据: ${error.message}`);
    }
