/*
 * 今日黄历&节假日倒数（V10.0 结构透视修复版）
 * ✅ 核心修复：增加 [路径A/路径B] 双重探测，防止年份文件夹结构变动
 * ✅ 核心修复：增加 [索引强制兜底]，匹配不到日期时，直接按第几天强行读取
 * ✅ 诊断功能：日志会打印 "抓取到的第一条数据"，方便肉眼比对
 */
(async () => {
  // ========== 1. 基础配置 ==========
  // 强制北京时间
  ...
  const fetchDeepData = async () => {
    // 路径策略 A: 标准结构 /2025/202501.json
    const pathA = `${BASE_URL}/${curYear}/${curYear}${monthStr}.json`;
    let res = await httpGet(pathA);

    // 路径策略 B: 扁平结构 /202501.json (防止年份文件夹未建立)
    if (res.status === 404) {
      log("⚠️ 路径A (年份文件夹) 404，尝试路径B...");
      const pathB = `${BASE_URL}/${curYear}${monthStr}.json`;
      res = await httpGet(pathB);
    }
  ...
  const getLunarDesc = async () => {
    ...
    if (result.error) return `⚠️ 数据获取失败: ${result.error}`;

    const rawJson = result.json;
    // 智能识别数据层级
    let list = [];
    if (Array.isArray(rawJson.days)) list = rawJson.days;
    else if (rawJson.data && Array.isArray(rawJson.data.days)) list = rawJson.data.days;
    else if (Array.isArray(rawJson)) list = rawJson;

    if (list.length === 0) return "⚠️ 数据源为空";

    // 🔍 诊断日志：打印第一条数据，确认结构
    log(`数据源首项样本: ${JSON.stringify(list[0])}`);

    // 🔥 三级匹配策略
    let target = null;

    // 1. 精确日期匹配 (2025-01-16)
    target = list.find(x => x.date === fullDateStr);
    
    // 2. Day字段数字匹配 (16 == 16)
    if (!target) {
      target = list.find(x => x.day && parseInt(x.day) === curDate);
    }

    // 3. 索引强制兜底 (第16天取数组第15个)
    if (!target) {
      const idx = curDate - 1;
      if (list[idx]) {
        log(`⚠️ 启用索引兜底 (Index: ${idx})`);
        target = list[idx];
      }
    }

    if (!target) return "📭 无法匹配今日数据";

    log(`✅ 最终匹配: ${JSON.stringify(target)}`);
    ...
    const yi = getV("yi", "Yi", "suit", "y");
    const ji = getV("ji", "Ji", "avoid", "j");
