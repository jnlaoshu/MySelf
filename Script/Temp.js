...
(async () => {
  ...
  try {
    ...
    const yi = get("yi","Yi","suit").replace(/\./g, " ");
    const ji = get("ji","Ji","avoid").replace(/\./g, " ");
    ...
    // 错误捕获，确保即使出错面板也能正常显示错误信息
    $done({ 
      title: "黄历小组件加载异常", 
      content: "请检查 Egern 运行日志: " + e.message,
