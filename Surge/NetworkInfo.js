...
function getIP() {
  ...
  if (!v4 && !v6) {
    ...
    if (v4?.primaryAddress) info.push(`本机：${v4?.primaryAddress}`);
   /* if (v6?.primaryAddress) info.push(`本机v6：${v6?.primaryAddress}`);*/
    if (v4?.primaryRouter && getSSID()) info.push(`路由：${v4?.primaryRouter}`);
   /*if (v6?.primaryRouter && getSSID()) info.push(`路由器：${v6?.primaryRouter}`);*/
  }
  info = info.join("\n");
  return info + "\n";
}
